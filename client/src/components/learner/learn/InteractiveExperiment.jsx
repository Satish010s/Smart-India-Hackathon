...
const SIM_RESULT = {
  ...
  histogram: [
    ...
    { state: '|11⟩', prob: 0.497, color: 'bg-[var(--color-primary)]' },
  ],
};
...
function CircuitBuilder({ circuit, onCircuitChange }) {
  ...
  return (
    <div className="rounded-xl border border-[var(--color-border)] bg-[#0d1117] overflow-hidden">
      <div className="px-4 py-2.5 border-b border-[var(--color-border)]/50 flex items-center justify-between">
        <span className="text-[10px] font-mono font-medium text-[var(--color-primary)] uppercase tracking-wider">Interactive Circuit Builder</span>
        ...
        <div className="space-y-2 py-2">
          {[{ label: 'q₀', gates: q0Gates }, { label: 'q₁', gates: q1Gates }].map((qubit, qi) => (
            <div key={qubit.label} className="flex items-center gap-3">
              <span className="text-xs font-mono text-[var(--color-text)] w-6 flex-shrink-0">{qubit.label}</span>
              <div className="flex items-center gap-1 flex-1 h-8">
                <div className="w-4 h-px bg-[var(--color-border)]" />
                {qubit.gates.map((g, i) => (
                  <React.Fragment key={i}>
                    <div className={`w-8 h-8 rounded-md flex items-center justify-center text-[10px] font-mono font-bold border transition-all cursor-pointer hover:scale-105 ${
                      g === 'CNOT'
                ...
                <div className="w-6 h-6 rounded border border-[var(--color-border)] flex items-center justify-center text-[9px] font-mono text-[var(--color-muted)]">M</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 sm:p-8">
        {onClose && (
          <button onClick={onClose} className="absolute top-4 right-4 p-2 rounded-lg hover:bg-[var(--color-border)]/40 text-[var(--color-muted)] transition-colors">
            <LuArrowLeft size={18} />
          </button>
        )}
        <div className="space-y-3 pr-10 pt-1">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-mono uppercase tracking-wider px-2.5 py-1 rounded border bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20">
              <LuFlaskConical size={10} className="inline mr-1" />Interactive Lab
            </span>
            <span className="text-[10px] font-mono text-[var(--color-muted)]">{completedSteps.length}/{experiment.steps.length} steps</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-semibold text-[var(--color-text)] tracking-tight">{experiment.title}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <button
              onClick={nextHint}
              className="flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400 hover:opacity-80 transition-opacity font-semibold"
            >
              <LuLightbulb size={13} /> {showHint ? 'Next hint' : 'Show hint'}
            </button>
            {showHint && (
              <div className="px-4 py-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                {HINTS[hintIdx]}
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden">
            <button
              ...
              className="w-full flex items-center gap-2.5 px-4 py-3 text-left hover:bg-[var(--color-border)]/20 transition-colors"
            >
              <LuBot size={15} className="text-[var(--color-primary)]" />
              <span className="text-xs font-semibold text-[var(--color-text)] flex-1">AI Assistant — need help?</span>
              <LuSparkles size={12} className="text-[var(--color-primary)]" />
            </button>
            {aiOpen && (
              <div className="px-4 pb-4 text-xs text-[var(--color-muted)] border-t border-[var(--color-border)] pt-3 leading-relaxed">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-[var(--color-border)] bg-[#0d1117] p-4">
            <div className="text-[10px] font-mono text-[var(--color-muted)] mb-3 uppercase tracking-wider">Circuit State</div>
            <pre className="text-sm font-mono text-emerald-300 leading-loose">{step.circuit}</pre>
          </div>
          ...
          <button
            ...
            disabled={simRunning}
            className="w-full py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-all text-white hover:opacity-90"
          ...
          {simDone && (
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-5 space-y-4">
              <div className="flex items-center gap-2">
                <LuCircleCheckBig size={16} className="text-emerald-500" />
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">Simulation Successful!</span>
              </div>
              <div className="text-xs font-mono text-[var(--color-muted)]">
                Statevector: <span className="text-emerald-600 dark:text-emerald-300">{SIM_RESULT.statevector}</span>
              </div>
              <div className="space-y-2">
              ...
              <div className="text-xs text-emerald-600 dark:text-emerald-300 font-mono">
          ...
          {/* Success state */}
          {allDone && simDone && (
            <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-5 text-center space-y-2">
              <div className="text-2xl">🎉</div>
              <div className="text-sm font-bold text-emerald-600 dark:text-emerald-400">Experiment Complete!</div>
